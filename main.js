$(document).ready(() => {
  ...
  let turn = true;
  let gameOver = false; // FIXME:

  // FIXME: space after if

  const checkWin = function checkWin() { // FIXME:
    for (let i = 0; i < winCells.length; i += 1) {
      ...
      if (winCountX >= winCountRequered) {
        $(`#total`).children().html('Win X'); // FIXME: semilicon needed
        console.log('win X!');
        gameOver = true; // FIXME: spaces
